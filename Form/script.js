...
function updateFormFields() {
    ...
    if (selectedServices.length > 0) {
        ...
        businessNatureInput.required = false;
    }
    ...
    if (requiresDirectors) {
        ...
        for (let i = 1; i <= num; i++) {
            ...
            directorDiv.innerHTML = htmlContent;
...
$(document).ready(function() {
    $('#business-form').on('submit', async function(e) {
        e.preventDefault();
        ...
        try {
            ...
            $.ajax({
                ...
                success: function(response) {
                    alert('Form submitted successfully! Check your Google Sheet and Drive.');
                    $('#business-form')[0].reset();
                    updateFormFields();
                    submitButton.prop('disabled', false).text('Submit');
                },
                error: function(xhr, status, error) {
                    alert('Error submitting form: ' + (xhr.responseJSON?.error || error));
                    submitButton.prop('disabled', false).text('Submit');
                }
            });
        } catch (error) {
...
numDirectorsPartnersInput.addEventListener('change', updateFormFields);
